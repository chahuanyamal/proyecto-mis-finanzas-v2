...
export default function ReconciliationPage() {
  ...
  return (
    <section className="p-4 text-slate-100 sm:p-6 lg:p-8"><div className="mx-auto max-w-6xl space-y-5">
      <header className="card p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.35em] text-brand-400">Control</p><h1 className="mt-2">Reconciliación</h1><p className="mt-2 text-sm text-slate-400">Compara saldo de cuenta contra neto de movimientos importados.</p></div><button className="btn-primary" onClick={() => setCurrency((c) => c === "CLP" ? "USD" : "CLP")}>{currency}</button></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><input className="rounded border border-slate-700 bg-black px-3 py-2 text-sm" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} /><input className="rounded border border-slate-700 bg-black px-3 py-2 text-sm" type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} /><input className="rounded border border-slate-700 bg-black px-3 py-2 text-sm" inputMode="decimal" value={tolerance} onChange={(e) => setTolerance(e.target.value)} placeholder="Tolerancia" /></div></header>
